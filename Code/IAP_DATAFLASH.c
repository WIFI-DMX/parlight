...
UINT8 read_APROM_BYTE(UINT16 code *u16_addr,UINT8 i)
{
	UINT8 rdata;
	rdata = *(u16_addr+i)>>8;
	return rdata;
...
void Read_APROM(void){
    UINT16 u16Count;    
    for(u16Count=0;u16Count<FMC_DAT_Length;u16Count++)
    {   
        Data[u16Count]=read_APROM_BYTE(DATA_START_ADDR,u16Count);     
    } 
}
